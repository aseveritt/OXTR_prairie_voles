...
mymat <- ddsnorm %>% 
    ...
    dplyr::select(-c(external_gene_name, rowname))
...
d0 <- bonding_df %>% 
  dplyr::select(c("ensembl_gene_id", colnames(bonding_df)[grep("log2FoldChange", colnames(bonding_df))])) %>%
...
d1 <- bonding_df %>%
  dplyr::select(c("ensembl_gene_id", colnames(bonding_df)[grep("padj", colnames(bonding_df))])) %>%
  tidyr::pivot_longer(cols = -c(ensembl_gene_id), values_to="padj", names_to="subgroup") %>%
  mutate(subgroup=gsub("padj_", "", subgroup)) 
d2 <- bonding_df %>%
  dplyr::select(c("ensembl_gene_id", colnames(bonding_df)[grep("lfcSE", colnames(bonding_df))])) %>%
...
d <- join(d, vole_annotation %>% 
            dplyr::select(ensembl_gene_id, external_gene_name), by="ensembl_gene_id")
...
for (pick in ofinterest){
  ...
  vect = bonding_df %>% 
    ...
    dplyr::select(c("padj_WTF", "padj_MUTF", "padj_WTM", "padj_MUTM")) %>%
    as.numeric() 
...
mydf_F <- res %>% as.data.frame() %>% 
  rownames_to_column(var="ensembl_gene_id") %>% 
  left_join(vole_annotation, by="ensembl_gene_id")
genotype_df$log2FC_F <- res$log2FoldChange; genotype_df$padj_F <- res$padj

res = lfcShrink(dds, type="ashr", contrast=c("group", "Mut6_M", "WT_M"), quiet = TRUE)  
mydf_M <- res %>% as.data.frame() %>% 
  rownames_to_column(var="ensembl_gene_id") %>% 
  left_join(vole_annotation, by="ensembl_gene_id")
genotype_df$log2FC_M <- res$log2FoldChange; genotype_df$padj_M <- res$padj
```
...
p2 <- suppressWarnings(treeplot(ego2, color="median_log2fc", showCategory = 65,
               cladelab_offset=30, tiplab_offset=2,
  nCluster = n,
...
n = 4
mycol= hue_pal()(n)
k_up <- min(n, nrow(ego_up))
p1 <- treeplot(ego_up, color="p.adjust", showCategory = 65,
  nCluster = k_up, group_color = mycol[seq_len(k_up)]) +  
  scale_fill_manual(values=rep(c("white"), n)) +
  scale_colour_continuous(low="cornflowerblue", high="red", name = "adjp", 
            guide = guide_colorbar(reverse = TRUE)) #+ theme(legend.position = "bottom")
p1$layers[[3]]$aes_params$size <- 3 # clusters
...
k_up <- min(n, nrow(ego_female), nrow(ego_male))
p1 <- treeplot(ego_female, color="p.adjust", showCategory = 65,
  nCluster = n, group_color = mycol[seq_len(k_up)]) +  
  scale_fill_manual(values=rep(c("white"), n)) +
  scale_colour_continuous(low="cornflowerblue", high="red", name = "adjp", 
            guide = guide_colorbar(reverse = TRUE)) #+ theme(legend.position = "bottom")
p1$layers[[3]]$aes_params$size <- 3 # clusters
...
```{r, fig.height=10, fig.width=12, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r message=FALSE, warning=FALSE,  fig.width=20, eval=FALSE}
